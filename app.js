...
async function fetchAndParseCSV(semLabel) {
    ...
    // 取得正確的 CSV 路徑
    let csvUrl = (CONFIG.SEMESTERS && semLabel && CONFIG.SEMESTERS[semLabel])
              || Object.values(CONFIG.SEMESTERS || {})[0]
              || './timetable_11501.csv';
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                else homeroomData = window.HOMEROOMS_EMBEDDED_JSON || {};
            } catch (e) { 
                homeroomData = window.HOMEROOMS_EMBEDDED_JSON || {}; 
            }
        } catch (netErr) {
            // 若為本地直接開啟 (file://) 被瀏覽器 CORS 限制，自動啟用內嵌資料庫
            if (window.TIMETABLE_EMBEDDED_CSV) {
                console.warn('偵測到本機檔案模式，已切換至內嵌課表資料庫');
                csvText = window.TIMETABLE_EMBEDDED_CSV;
                homeroomData = window.HOMEROOMS_EMBEDDED_JSON || {};
            } else {
                throw netErr;
            }
        ...
        if (badge) badge.textContent = semLabel || Object.keys(CONFIG.SEMESTERS || {})[0] || '115學年度第1學期';
        ...
        console.error(err);
        document.getElementById('loginError').textContent =
            `載入失敗：${err.message}。請確認資料檔案路徑或啟動本地伺服器。`;
    }
}
